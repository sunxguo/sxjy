<!DOCTYPE html>
<head>
<meta charset="UTF-8">
<title>市县</title>
<link rel="stylesheet" type="text/css" href="/assets/plugins/swiper/swiper.min.css">
<link rel="stylesheet" href="/assets/css/denglu.css"/>
</head>
<body>
	<?php require('head.php');?>
	<div style="background:#fde2cf; height:40px;">
		<div style="width:1000px; margin:0 auto;">
		    <ul style="float:left;text-align:center;">
				<li style="float:left; padding:11px 0; width:100px;">
					<a href="/" style="font-size:14px; ">首页</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px;">
					<a href="/home/news" style="font-size:14px;">新闻</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px;  ">
					<a href="/home/edu" style="font-size:14px;">教育</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px; background:#ff6600;">
					<a style="font-size:14px; color:#ffffff;">市县</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px;">
					<a href="/home/enrol" style="font-size:14px;">招考</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px;">
					<a href="/home/school" style="font-size:14px;">学校</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px;">
					<a href="/home/policy" style="font-size:14px;">政策</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px;">
					<a href="/home/active" style="font-size:14px;">活动</a>
				</li>
				<li style="float:left; padding:11px 0; width:100px;">
					<a href="/home/about" style="font-size:14px;">关于</a>
				</li>
				<div style="clear:both;"></div>
			</ul>
	    </div>	
	</div>
	<div style="background:#f66d10; height:48px;">
			<div style="width:1000px; margin:0 auto;">
				<div style="text-align:left; padding-top:8px;"> 
					<ul>
					    <li style="float:left;">
							<a href="/home/area" class="channel">市 县 频 道</a>
						</li>
						<li style="float:left; margin-left:26px; margin-top:10px;">	
							<a href="/home/areaSub" style="color:#ffffff;">太 原</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">临 汾</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">运 城</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">晋 城</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">长 治</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">阳 泉</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">晋 中</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">忻 州</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">朔 州</a>
							<b style="color:#ffffff;">丨</b>
							<a href="/home/areaSub"  style="color:#ffffff;">吕 梁</a>
						</li>
						<div style="clear:both;"></div>
					</ul>
				</div>
			</div>
	</div>
	<div style="background:url(/assets/images/63.png); height:426px;">
		
		<div id="swiperNews" class="swiperNews areaBanner">
			<div class="hd">
				<ul><li>1</li><li>2</li><li>3</li></ul>
			</div>
			<div class="bd">
				<ul>
					<li><a href="detail.html" target="_blank"><img src="/assets/images/59.png" /></a></li>
					<li><a href="detail.html" target="_blank"><img src="/assets/images/59.png" /></a></li>
					<li><a href="detail.html" target="_blank"><img src="/assets/images/59.png" /></a></li>
				</ul>
			</div>
		</div>
		<!-- <div style="background:url(/assets/images/59.png); height:426px; margin:0 auto; width:1000px;"></div> -->
	</div>
	<div>		
		<div style="width:1000px; margin:0 auto;">	
			<div style="float:left;">
				<img src="/assets/images/64.png" style="padding:20px 0;"/>
				<div style="width:292px; border:8px solid #eeeeee; height:556px;">
					<div style="padding:10px 10px; font-size:18px;">
						<p>图文动态</p>
						<div style="border-top:1px dashed #d4d4d4; margin-top:10px;"></div>
						<img src="/assets/images/60.png" style="padding:10px 25px;"/>
						<img src="/assets/images/61.png" style="padding:10px 25px;"/>
						<img src="/assets/images/62.png" style="padding:10px 25px;"/>
					</div>
				</div>
			</div>
			<div style="float:right;">
				<img src="/assets/images/65.png" style="padding:20px 0;"/>
				<div style="width:654px;">
					<ul style="margin-top:18px;">
						<li style="float:left;">
							<a>全部动态</a>
						</li>
						<li  style="text-align:right; float:right;">
							<a>更多》</a>
						</li>
						<div style="clear:both;"></div>
					</ul>
				</div>
				<div style="border-top:1px dashed #d4d4d4; margin-top:10px;"></div>
				<div style="padding:10px 0;">
					<ul>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】原师院中区月底前搬迁　原址重新规划</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】我省2014年硕士研究生招生考试成绩揭晓</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【长治】长治市在全市中小学校深入开展“三爱”教育活动</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】山西2013年贫困大学新生可申请“圆梦”基金</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】山西2013年贫困大学新生可申请“圆梦”基金</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】山西2013年贫困大学新生可申请“圆梦”基金</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】山西2013年贫困大学新生可申请“圆梦”基金</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
						</ul>
				</div>
			</div>
			<div style="clear:both;"></div>
		</div>
	</div>
	<div>
		<img src="/assets/images/66.png" style="padding:20px 0;"/>
	</div>
	<div>
		<div style="width:1000px; margin:0 auto;">
			<div style="float:left;">
				<div style="width:304px; border:2px solid #d4d4d4;">
					<div style="padding:10px 10px; font-size:18px;">
						<p>图文动态</p>
						<div style="border-top:1px dashed #d4d4d4; margin-top:10px;"></div>
						<ul style="padding:10px 0;">
							<li style="padding:15px 0; background:#ff6600;">
								<a style="float:left; margin-left:10px; color:#ffffff;">太原市</a>
								<a style="float:right; color:#ffffff;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">运城市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">晋城市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">长治市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">阳泉市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">晋中市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">忻州市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">大同市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">朔州市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:15px 0;">
								<a style="margin-left:10px;">吕梁市</a>
								<a style="float:right;">》</a>
								<div style="clear:both;"></div>
							</li>
						</ul>
					</div>
				</div>
			</div>
			<div style="float:right;">
				<div style="width:654px; border:2px solid #d4d4d4;">
					<div style="padding:10px 0;">
						<ul>
							<li style="padding:10px 10px;">
								<a style="float:left;"></a>
								<a style="float:right;">更多》</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】原师院中区月底前搬迁　原址重新规划</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】我省2014年硕士研究生招生考试成绩揭晓</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【长治】长治市在全市中小学校深入开展“三爱”教育活动</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】山西2013年贫困大学新生可申请“圆梦”基金</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】山西2013年贫困大学新生可申请“圆梦”基金</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【太原】山西2013年贫困大学新生可申请“圆梦”基金</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】为评院士花2300万打通关系？中科院回应对受贿院士不姑息</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】教育部：超4亿 近三成国人不能用普通话交流</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
							<li style="padding:6px 10px;">
								<a style="float:left;">【地市动态】中国人才无端消耗：本科生端盘子 博士生跑销售</a>
								<a style="float:right;">2015年7月2日</a>
								<div style="clear:both;"></div>
							</li>
						</ul>
				    </div>
				</div>
			</div>
		</div>
	</div>
	<div>
		<img src="/assets/images/66.png" style="padding:20px 0;"/>
	</div>
	<?php require('foot.php');?>
</body>
<script src="/assets/js/SuperSlide.js"></script>
<script type="text/javascript">
jQuery(".swiperNews").slide({mainCell:".bd ul",autoPlay:true,trigger:"click"});
</script>
</html>